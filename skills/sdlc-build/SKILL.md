...
用户说“继续”“开始实现”“按计划执行”或等价指令后：

1. 同一回合立即执行至少一个真实工具动作：读取 STATE/plan/目标源码或测试、创建或修改测试、运行测试/构建/检查之一。
2. 不得只回复“收到，我开始执行”“下一轮继续”“我准备先……”便结束回合。
3. 声称“已开始/已完成/已验证”时，必须同时给出实际文件、命令结果或明确阻塞证据。
4. 上下文压缩、历史过长或需要重读文件不属于 BLOCKED；从 STATE、plan、目标源码和当前 Git diff 恢复。
5. 任务过大时自动拆成最小闭环：一个行为 → 一个失败测试 → 确认失败 → 最小实现 → 确认通过。
6. 只有当前任务完成且有验证证据、遇到真实 BLOCKED、触发既定门控或全部计划完成时，才允许结束执行回合。
7. 若上一回合只有文字而无真实动作，本回合必须先恢复执行，不能再次要求用户输入“继续”。
